...
public class FarmDriver {
    public static void main(String[] args) {
        ...
        crops.add(new Crop("Turnip", 1, 2, 0, 1, 
                2, 1, 2, 6, 5, 5));
        ...
        while (canContinue) {
            ...
            if (mainFarm.getPlayer().getWallet() == 0 && !mainFarm.getTiles().get(0).isOccupied() && !mainFarm.getTiles().get(0).isRock()) {
                System.out.println("\nYou ran out of money.");
                canContinue = false;
            }

            if (mainFarm.getTiles().get(0).getCrop() != null) {
                if (mainFarm.getTiles().get(0).getCrop().isWithered()) {
                    System.out.println("\nYour plant withered.");
                    canContinue = false;    
                }
            }
